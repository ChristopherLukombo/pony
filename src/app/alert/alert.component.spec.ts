...
@Component({
  ...
  template: `<pr-alert (closed)="closed = true; event = $event">Hello</pr-alert>`
})
...
describe('AlertComponent', () => {
  ...
  it('should be created with default values', () => {
    ...
    expect(component.type).withContext('The type input should be `warning` by default').toBe('warning');
    expect(component.dismissible).withContext('The dismissible input should be `true` by default').toBe(true);
  });
  ...
  it('should compute the classes to apply', () => {
    ...
    expect(component.alertClasses)
      .withContext('The alertClasses getter should depend on the type and dismissible inputs')
      .toContain('alert');
    expect(component.alertClasses)
      .withContext('The alertClasses getter should depend on the type and dismissible inputs')
  ...
  it('should apply the classes on the root element', () => {
    ...
    expect(div.className).withContext('The alertClasses should be applied on the root element').toContain('alert');
    expect(div.className).withContext('The alertClasses should be applied on the root element').toContain('alert-dismissible');
    expect(div.className).withContext('The alertClasses should be applied on the root element').toContain('alert-warning');
  });
